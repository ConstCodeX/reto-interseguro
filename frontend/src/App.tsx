...
function ArchitectureView() {
  ...
  return <section className="architecture-view">
    <div className="architecture-heading"><div><div className="eyebrow"><Layers3 size={13} /> ARQUITECTURA TO-BE</div><h2>Reto 3</h2><p>Diseño resiliente para evitar desembolsos duplicados cuando INARI demora o responde 502.</p></div><a className="architecture-link" href="/arquitectura-desembolsos.drawio" download="arquitectura-desembolsos.drawio"><Download size={13} /> descargar diagrama .drawio</a></div>
    ...
    <div className="architecture-explanation"><div className="eyebrow"><Sparkles size={13} /> COMO RESOLVÍ EL PROBLEMA</div><p className="architecture-problem">El flujo original era síncrono: el cliente esperaba la respuesta de INARI y, ante un 502 o un timeout, no sabía si el desembolso se había ejecutado o no. Al reintentar manualmente, la misma póliza podía procesarse dos veces. Rediseñé el flujo para que cada paso sea idempotente, asíncrono y observable, sin modificar INARI.</p><div className="explanation-grid">
      <div className={activeSolution === 'duplicates' ? 'explanation-active' : ''} onMouseEnter={() => setActiveSolution('duplicates')} onMouseLeave={() => setActiveSolution(null)}><b>01 / Evito duplicados</b><span>El frontend genera una <code>Idempotency-Key</code> por intento de desembolso. El backend la usa como clave para adquirir un lock distribuido en Firestore/Redis por póliza o transacción: si ya existe una solicitud en curso para esa clave, no creo un nuevo registro, devuelvo el <code>requestId</code> existente. Así, un doble clic o un reintento tras el 502 nunca generan un segundo desembolso.</span></div>
      <div className={activeSolution === 'fast' ? 'explanation-active' : ''} onMouseEnter={() => setActiveSolution('fast')} onMouseLeave={() => setActiveSolution(null)}><b>02 / Respondo rápido</b><span>En vez de esperar a INARI, el backend valida la solicitud, registra el estado <code>PENDING</code> y responde <code>202 Accepted</code> con el <code>requestId</code> en milisegundos. El trabajo pesado se publica en Cloud Tasks/Pub-Sub. Esto evita los timeouts del proxy que originaban el 502 y desacopla la disponibilidad del frontend de la latencia del sistema legado.</span></div>
      <div className={activeSolution === 'retries' ? 'explanation-active' : ''} onMouseEnter={() => setActiveSolution('retries')} onMouseLeave={() => setActiveSolution(null)}><b>03 / Reintento con control</b><span>Un Worker en Cloud Run consume la tarea, revalida la idempotencia y recién ahí llama a INARI. Si responde 502 o hay timeout, la cola reintenta automáticamente con backoff exponencial y un número máximo de intentos. Cuando se agotan, el evento va a una Dead Letter Queue para revisión manual, así ninguna transacción se pierde silenciosamente.</span></div>
      <div className={activeSolution === 'notify' ? 'explanation-active' : ''} onMouseEnter={() => setActiveSolution('notify')} onMouseLeave={() => setActiveSolution(null)}><b>04 / Informo el resultado</b><span>Persisto el estado (<code>PENDING</code> → <code>PROCESSING</code> → <code>SUCCEEDED</code>/<code>FAILED</code>) en Firestore junto al <code>requestId</code>. El frontend se suscribe a esos cambios por SSE o WebSocket, con polling como respaldo, así el usuario ve el resultado real del desembolso aunque INARI tarde varios segundos en contestar.</span></div>
      <div className={activeSolution === 'security' ? 'explanation-active' : ''} onMouseEnter={() => setActiveSolution('security')} onMouseLeave={() => setActiveSolution(null)}><b>05 / Protejo la comunicación</b><span>La API exige un JWT válido en cada solicitud, igual que el resto de los retos. Para hablar con INARI (que vive en una red interna) el Worker sale por un VPC Connector, así el sistema legado nunca queda expuesto a internet y el tráfico entre Cloud Run e INARI se mantiene dentro de la red privada del proyecto.</span></div>
    </div></div>
  </section>
}
